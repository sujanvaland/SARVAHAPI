...
namespace SpiritualNetwork.API.Services
{
    public class SearchService : ISearchService
    {
        ...
        public SearchService(IRepository<User> userRepository, IRepository<OnlineUsers> onlineuserRepository)
        {
            ...
            try 
            {
                if (Name.Length > 0)
                {
                    var data = await (from user in _userRepository.Table
                                      ...
                                      where user.UserName.Contains(Name) || user.FirstName.Contains(Name) ||                          user.LastName.Contains(Name) && user.IsDeleted == false
                                      select new
                                      {
                                          user.Id,
                                          user.FirstName,
